...
export default function AnalysisReview({ speechMetrics, language }: AnalysisReviewProps) {
  ...
  // Animate score and charts on mount
  useEffect(() => {
    const duration = 1500;
  ...
  const getEncouragement = (score: number) => {
    if (language === 'ko') {
      if (score >= 85) return {
        title: '정말 멋져요!',
        message: '원어민 수준의 유창함을 보여주셨어요. 이 실력을 계속 유지하세요!',
        gradient: 'from-emerald-400 to-teal-500'
      };
      if (score >= 70) return {
        title: '훌륭한 실력이에요!',
        message: '자신감 있게 대화하는 모습이 인상적이에요. 조금만 더 연습하면 완벽해요!',
        gradient: 'from-cyan-400 to-blue-500'
      };
      if (score >= 55) return {
        title: '좋은 출발이에요!',
        message: '기초가 탄탄해요. 꾸준히 연습하면 더 유창해질 거예요!',
        gradient: 'from-violet-400 to-purple-500'
      };
      return {
        title: '잘 하고 있어요!',
        message: '대화를 시작한 것만으로도 큰 발걸음이에요. 계속 도전하세요!',
        gradient: 'from-pink-400 to-rose-500'
      };
    } else {
      if (score >= 85) return {
        title: 'Outstanding!',
        message: "You're speaking with near-native fluency. Keep up this excellent work!",
        gradient: 'from-emerald-400 to-teal-500'
      };
      if (score >= 70) return {
        title: 'Great Job!',
        message: "You're conversing with confidence. A bit more practice and you'll be perfect!",
        gradient: 'from-cyan-400 to-blue-500'
      };
      if (score >= 55) return {
        title: 'Good Start!',
        message: "Your foundation is solid. Keep practicing and you'll see rapid improvement!",
        gradient: 'from-violet-400 to-purple-500'
      };
      return {
        title: "You're Doing Great!",
        message: 'Starting the conversation is a huge step. Keep challenging yourself!',
        gradient: 'from-pink-400 to-rose-500'
      };
    }
  };
  ...
  // Sort metrics for strengths/weaknesses
    ...
    <div className="space-y-4">
          ...
          <div className="flex flex-col items-center">
            <div className="relative w-52 h-52 sm:w-60 sm:h-60">
              <svg className="w-full h-full transform -rotate-[135deg]" viewBox="0 0 240 240">
                ...
                <circle
                  cx="120" cy="120" r={arcRadius}
                  fill="none" stroke="url(#arcGrad)" strokeWidth="16"
                  strokeDasharray={`${arcProgress} ${circumference}`}
                  strokeLinecap="round"
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-6xl sm:text-7xl font-bold bg-gradient-to-br from-cyan-200 via-purple-200 to-pink-200 bg-clip-text text-transparent leading-none">
                  {displayScore}
                </span>
            ...
            <div className="text-center mt-4 max-w-sm">
              <h4 className={`text-xl sm:text-2xl font-bold bg-gradient-to-r ${encouragement.gradient} bg-clip-text text-transparent mb-2`}>
                {encouragement.title}
              </h4>
      ...
      <div className="rounded-2xl bg-slate-900/80 border border-white/[0.06] p-4 sm:p-5">
          ...
          <svg width={radarSize} height={radarSize} viewBox={`0 0 ${radarSize} ${radarSize}`}>
            ...
            {dataPoints.map((pt, i) => (
              <circle
                ...
                fill={metrics[i].level === 'high' ? '#10b981' : metrics[i].level === 'medium' ? '#06b6d4' : '#f59e0b'}
            ...
            {metrics.map((m, i) => {
              ...
              return (
                <text
                  ...
                  fill="rgba(255,255,255,0.45)" fontSize="9" fontWeight="500"
                >
                  {m.label}
                </text>
              );
        ...
        <p className="text-center text-slate-500 text-[10px] sm:text-xs">
          {language === 'ko'
            ? '모든 영역에서 고르게 성장하는 것이 중요해요'
            : 'Balanced growth across all areas is the key'}
        </p>
      </div>
      ...
      {/* ====== 4. DETAILED METRICS: Horizontal Bars ====== */}
      <div className="rounded-2xl bg-slate-900/80 border border-white/[0.06] p-4 sm:p-5">
      ...
      <div className="rounded-2xl bg-purple-500/[0.05] border border-purple-500/[0.08] p-4 sm:p-5">
        ...
        <div className="space-y-2">
          ...
          {/* Growth insights */}
          {growthAreas.map((g) => (
            <p key={`g-${g.label}`} className="text-xs sm:text-sm text-slate-300 flex items-start gap-2">
