    ...
    >
      ...
      <div
        className={`
          ...
          ${
            isUnlocked
              ? 'bg-gradient-to-br from-violet-500/10 to-indigo-500/10 dark:from-violet-500/20 dark:to-indigo-500/20 border-2 border-violet-400/40 dark:border-violet-400/30 shadow-lg shadow-violet-500/10'
      ...
      >
        ...
        <svg
          className={`${size === 'sm' ? 'w-5 h-5' : size === 'md' ? 'w-7 h-7' : 'w-9 h-9'} ${
            isUnlocked ? 'text-violet-500' : 'text-neutral-400 opacity-30 dark:opacity-20'
          }`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        ...
        {/* Unlocked glow ring */}
        {isUnlocked && (
          <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-violet-500 to-indigo-500 opacity-15 blur-md -z-10 animate-glow" />
        )}
      </div>
      ...
      {showTooltip && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 pointer-events-none">
          <div className="px-3 py-2 rounded-xl card-premium text-center whitespace-nowrap shadow-[var(--card-shadow-hover)]">
            <p className={`font-semibold text-theme-primary ${dimensions.tooltip}`}>
              {achievement.name[language]}
            </p>
            <p className={`text-theme-muted mt-0.5 ${size === 'sm' ? 'text-2xs' : 'text-xs'}`}>
              {achievement.description[language]}
            </p>
            {!isUnlocked && (
              <p className="text-violet-500 dark:text-violet-400 text-2xs mt-1 font-medium">
                Locked
              </p>
            )}
          </div>
